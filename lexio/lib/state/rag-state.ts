...
const allowedActionReturnValues: Record<UserAction['type'], string[]> = {
    ADD_USER_MESSAGE: ['response', 'sources', 'citations', 'setUserMessage', 'followUpAction'],
    ...
    async (
        ...
        const processCitations = async () => {
            ...
            const citationsData = await addTimeout(
                Promise.resolve(response.citations), // Handle both direct array and Promise<array>
        ...
        try {
            const results = await Promise.allSettled([processSources(), processMessage(), processCitations()]);
            ...
            // Apply citations to the last message if they were provided and processed successfully
            if (results[2]?.status === 'fulfilled' && results[2]?.value) {
                const citationsData = results[2].value;
                const messages = get(completedMessagesAtom);
                const lastMessage = messages[messages.length - 1];

                if (lastMessage && lastMessage.role === 'assistant') {
                    // Update the last message with citations
                    set(completedMessagesAtom, messages.map(msg =>
