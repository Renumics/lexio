...
// Mock function
const mockRetrieveAndGenerateFn = vi.fn((query, metadata) => ({
...
describe('Atom State Tests', () => {
  it('should handle basic functionality and update atoms', async () => {
    ...
    let response;
    await act(async () => {
      response = retrieveResult.current[1]({ query: 'test' });
  ...
  it('should handle errors correctly and update errorAtom', async () => {
    ...
    await act(async () => {
      const response = retrieveResult.current[1]({ query: 'test' });
  ...
  it('should handle timeouts correctly and update errorAtom', async () => {
    ...
    const retrieveAndGenerateAtom = createRetrieveAndGenerateAtom(() => ({
      sources: new Promise(() => {}),
    ...
    await act(async () => {
      retrieveResult.current[1]({ query: 'test' });
    });
