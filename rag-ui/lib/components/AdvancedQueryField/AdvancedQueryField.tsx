...
const AdvancedQueryField: React.FC<AdvancedQueryFieldProps> = ({
  ...
  const insertMention = useCallback(
    (source: Source) => {
      ...
      const findTextNodeWithPosition = (node: Node, position: number): Node | null => {
        if (node.nodeType === Node.TEXT_NODE) {
          return node;
        }
        for (let i = 0; i < node.childNodes.length; i++) {
          const found = findTextNodeWithPosition(node.childNodes[i], position);
          if (found) return found;
        }
        return null;
      };

      const textNode = findTextNodeWithPosition(editorElement, triggerPosition);
      ...
      const beforeText = textNode.textContent?.slice(0, triggerPosition) || '';
      const afterText = textNode.textContent?.slice(triggerPosition + 1) || '';
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    if (e.key === '@') {
      ...
      setTriggerPosition(range.startOffset);
